...
public class StrongRealismTest {
	...
	public void prologTest()
			throws MalformedGoalException, InvalidTheoryException, UnknownVarException, NoSolutionException {
	...
	private static String getSense(String literal) {
		return "sense(" + literal.substring(0, literal.length() - 1) + ").";
	}

	public static void main(String[] args) {
		plansContext.getInstance().appendFact("plan(check(slots),_6317,[garbage,(\\+ garbage)],_6318).");
		plansContext.getInstance()
				.appendFact("plan(check(slots),_6321,[(\\+ garbage),garbage,(\\+ clear),clear],_6322).");
		desiresContext.appendFact("check(slots).");

		String literal = "not clear.";
		...
		communicationContext.appendFact(getSense(literal));
		...
		BridgeRule r2 = BridgeRule.builder().head(Head.builder().context(beliefsContext).clause("X").build()).body(body)
				.build();
	...
	private BridgeRule getBridgeRule1StrongRealism() {
		...
		return BridgeRule.builder().head(Head.builder().context(beliefsContext).clause("X").build()).body(body).build();
	...
	public void testaRegraParaStrongRealism2() {
		...
		plansContext.getInstance().appendFact(
				"plan(check(slots),_,[(\\+ garbage),garbage,(\\+ clear),clear],_)." + System.lineSeparator());
	...
	public void testaCrenca() {
		...
		BridgeRule.builder().head(Head.builder().clause("P").context(DesiresContextService.getInstance()).build())
				.body(Body.builder().context(BeliefsContextService.getInstance()).clause("at(P)").build()).build()
				.execute();
	...
	public void testaRegrasNegacao() {
		...
		/// Head head =
		/// Head.builder().context(beliefsContext).clause("~breeze(X,Y)").build();
		// Body body =
		/// Body.builder().context(communicationContext).clause("sense(breeze)")
		// .and(Body.builder().context(beliefsContext).clause("pos(X,Y)").build()).build();
		// BridgeRule.builder().head(head).body(body).build().execute();
